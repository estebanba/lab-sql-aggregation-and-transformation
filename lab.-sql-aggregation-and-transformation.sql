...
SELECT DATEDIFF(MAX(rental.rental_date), MIN(rental.rental_date)) AS operating_days FROM rental;
...
-- SELECT rating, ROUND(AVG(length),2) AS average_duration FROM film WHERE length > 120 GROUP BY rating ORDER BY average_duration DESC;
